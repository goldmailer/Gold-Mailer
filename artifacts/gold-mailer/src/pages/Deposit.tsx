            ...
            {depositMethod === "crypto" ? (
              ...
              <div className="space-y-4">
                <div className="rounded-2xl border border-border bg-background/50 p-4">
                  {accountLoading ? <div className="h-20 animate-pulse rounded-lg bg-muted" /> : !myAccount ? <p className="text-sm text-muted-foreground">{t("deposit.notConfigured")}</p> : myAccount.type === "bank" && myAccount.accountNumber ? <div className="space-y-2"><div className="flex justify-between text-sm"><span className="text-muted-foreground">Bank</span><b>{myAccount.bankName}</b></div><div className="flex justify-between text-sm"><span className="text-muted-foreground">Account name</span><b>{myAccount.accountName}</b></div><div className="mt-3 flex items-center justify-between rounded-xl border border-primary/25 bg-primary/10 p-3"><div><p className="text-xs text-muted-foreground">Account number</p><p className="font-mono font-bold text-primary">{myAccount.accountNumber}</p></div><CopyButton value={myAccount.accountNumber} id="bank-account" /></div></div> : myAccount.type === "paypal" && myAccount.paypalEmail ? <div className="flex items-center justify-between text-sm"><span className="text-muted-foreground">PayPal email</span><span className="flex items-center gap-2 font-semibold">{myAccount.paypalEmail}<CopyButton value={myAccount.paypalEmail} id="paypal-email" /></span></div> : <p className="text-sm text-muted-foreground">Deposit details are not configured yet.</p>}
                </div>
