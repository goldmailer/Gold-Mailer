      ...
      <main className="pl-16 pt-16 max-w-xl mx-auto px-4 py-8">
        ...
        <div className="bg-card border border-border rounded-2xl p-6 mb-4">
          <div className="flex items-center gap-3 mb-5">
            ...
            <h2 className="font-bold">{isNGN ? "Transfer to this account" : "Send payment via PayPal"}</h2>
          </div>
          ...
          {accountLoading ? (
            ...
            !account?.accountNumber ? (
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center p-3 bg-primary/10 border border-primary/30 rounded-lg">
                  ...
                  <button onClick={() => copyValue(account.accountNumber!, "Account number")} className="p-2 rounded-lg bg-primary/20 hover:bg-primary/30 transition-colors">
            ...
            !(account as any)?.paypalEmail ? (
              <p className="text-muted-foreground text-sm">PayPal deposit not configured yet. Please check back later.</p>
            ) : (
              <div className="space-y-3">
                <div className="bg-blue-500/10 border border-blue-500/30 rounded-xl p-4">
                  ...
                  <div className="flex justify-between p-3 bg-background rounded-lg mb-2">
                    ...
                    <span className="font-semibold text-sm">{(account as any).paypalName ?? "—"}</span>
                  </div>
                  <div className="flex justify-between items-center p-3 bg-primary/10 border border-primary/30 rounded-lg">
                    <div>
                      <p className="text-xs text-muted-foreground">PayPal Email</p>
                      <p className="font-black text-primary">{(account as any).paypalEmail}</p>
                    </div>
                    <button onClick={() => copyValue((account as any).paypalEmail!, "PayPal email")} className="p-2 rounded-lg bg-primary/20 hover:bg-primary/30 transition-colors ml-3">
                      {copied ? <Check size={18} className="text-green-400" /> : <Copy size={18} className="text-primary" />}
                    </button>
                  </div>
                </div>
        ...
        {step === 2 && (
          <div className="bg-card border border-border rounded-2xl p-6">
            <div className="flex items-center gap-3 mb-5">
              ...
              <h2 className="font-bold">{isNGN ? "Paste your Transaction ID" : "Paste your PayPal Transaction ID"}</h2>
            </div>
            <div className="mb-4">
              ...
              <Input value={txId} onChange={e => setTxId(e.target.value)}
                placeholder={isNGN ? "Paste your bank transaction reference here" : "Paste your PayPal transaction ID here"} />
              <p className="text-xs text-muted-foreground mt-2">
                {isNGN
                  ? "Find this in your bank app under transaction history"
                  : "Find this in your PayPal activity under payment details"}
              </p>
            </div>
            <Button
